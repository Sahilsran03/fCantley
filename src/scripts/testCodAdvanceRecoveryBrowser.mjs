...
const fixture = { order: null, methodRequests: [], creates: 0, initiations: 0, verifyMode: "uncertain", codAvailable: true, codAdvance: 100, paid: false, codVerifications: 0, codInitiations: 0 };
const runtimeErrors = [];
...
const handleRequest = async ({ requestId, request }) => {
  ...
  if (path === "/orders/order-1/payments/cod-advance") {
    fixture.codInitiations++;
    if (fixture.reconcileOnInitiate) {
      fixture.order = { ...fixture.order, codAdvancePayment: "internal-cod", paymentStatus: "Paid",
        onlineAmountPaid: fixture.codAdvance, remainingCodDue: 0 };
      return fulfill(requestId, { status: "PAYMENT_CONFIRMED", orderId: "order-1", paymentId: "internal-cod" });
    }
    return fulfill(requestId, { status: "PAYMENT_INITIATED", keyId: "public_test_key", razorpayOrderId: "provider-cod",
      paymentId: "internal-cod", amount: fixture.codAdvance, currency: "INR", cantleyOrderId: "order-1" });
  }
  if (path === "/orders/order-1/payments/cod-advance/verify") {
    fixture.codVerifications++;
    assert.deepEqual(Object.keys(data).sort(), ["razorpay_order_id", "razorpay_payment_id", "razorpay_signature"]);
    if (fixture.codVerifications === 1) return fulfill(requestId, { message: "Simulated verification transport uncertainty" }, 500);
    fixture.order = { ...fixture.order, codAdvancePayment: "internal-cod", paymentStatus: "Paid",
      onlineAmountPaid: fixture.codAdvance, remainingCodDue: 0 };
    return fulfill(requestId, { status: "PAYMENT_CONFIRMED", orderId: "order-1", paymentId: "internal-cod" });
  }
...
try {
  ...
  await evaluate("document.querySelector('.order-success-payment .primary-button').click()");
  await waitFor("window.__payments?.length === 1");
  await evaluate("window.__payments[0].options.handler({razorpay_order_id:'provider-cod',razorpay_payment_id:'cod-payment',razorpay_signature:'test-signature'});window.__payments[0].options.handler({});window.__payments[0].close()");
  await waitFor("document.body.textContent.includes('Payment status could not be confirmed')");
  assert.equal(fixture.codVerifications, 1, "duplicate callbacks cannot verify twice");
  assert.equal(await evaluate("Boolean(document.querySelector('.order-success-payment .primary-button'))"), false);
  assert.equal(fixture.codInitiations, 1);
  await evaluate("document.querySelector('.order-success-payment .secondary-button').click()");
  await waitFor("document.body.textContent.includes('Advance payment confirmed.')");
  assert.equal(fixture.codVerifications, 2);
  assert.equal(fixture.codInitiations, 1, "recovery verifies the same payment; no second initiation");
  assert.equal(await evaluate("window.__payments.length"), 1);
  assert.equal(await evaluate("Boolean(document.querySelector('.order-success-payment .primary-button'))"), false);
  fixture.reconcileOnInitiate = true;
  fixture.order = { ...fixture.order, paymentStatus: "Pending", onlineAmountPaid: 0, remainingCodDue: 650, codAdvancePayment: null };
  await command("Page.navigate", { url: "http://127.0.0.1:5183/order-success/order-1" });
  await waitFor("document.querySelector('.order-success-payment .primary-button')");
  await evaluate("document.querySelector('.order-success-payment .primary-button').click()");
  await waitFor("document.querySelector('.order-success-payment').textContent.includes('Advance payment confirmed.')");
  assert.equal(await evaluate("Boolean(window.__payments?.length)"), false, "server-reconciled initiation never opens Razorpay");
  assert.equal(await evaluate("Boolean(document.querySelector('.order-success-payment .primary-button'))"), false);
  assert.deepEqual(runtimeErrors, []);
  console.log("COD browser recovery passed: verification 500, pending status refresh stays locked, same-callback verification recovery, duplicate/dismissal guard and no second popup.");
